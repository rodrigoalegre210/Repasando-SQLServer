...
CREATE VIEW vista_club AS
	SELECT s.nombre AS Nombre, s.documento AS Documento, s.domicilio AS Domicilio,
	c.deporte AS Deporte, c.dia AS Dia, p.nombre AS Profesor, i.matricula AS Matricula
	FROM socios AS s
...
-- 6
SELECT Deporte, Dia, COUNT(Nombre)AS Cantidad
FROM vista_club
...
CREATE VIEW vista_inscriptos AS
	SELECT deporte AS Deporte, dia AS Dia,
