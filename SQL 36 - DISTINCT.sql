...
-- 4
SELECT DISTINCT provincia AS Provincias
FROM clientes;
...
-- 8
SELECT DISTINCT ciudad AS 'Ciudades de Córdoba'
FROM clientes
...
-- 6
SELECT COUNT(DISTINCT documento) AS 'Cantidad de Propietarios'
FROM inmuebles
...
FROM inmuebles
WHERE domilico LIKE '%San Martin%';

-- 8
SELECT DISTINCT apellido AS Apellido,
nombre AS Nombre
FROM inmuebles;

-- 9
SELECT documento AS Documento,
COUNT(DISTINCT barrio) AS Propiedades
FROM inmuebles
GROUP BY documento;