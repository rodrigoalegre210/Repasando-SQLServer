...
-- 4
SELECT DISTINCT d1.profesor AS Profesor
FROM deportes AS d1
...
SELECT DISTINCT d1.profesor AS Profesor
FROM deportes AS d1
JOIN deportes AS d2 ON d1.profesor = d2.profesor
WHERE d1.nombre <> d2.nombre;
...
dia = (SELECT dia FROM deportes
WHERE nombre = 'natacion');

-- 7
SELECT d1.nombre AS Nombre
FROM deportes AS d1
JOIN deportes AS d2 ON d1.dia = d2.dia
